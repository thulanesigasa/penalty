...
class TestPenaltyEnv(unittest.IsolatedAsyncioTestCase):
    """
    Unit tests for the asynchronous Penalty Game Gymnasium-compatible environment.
    Uses IsolatedAsyncioTestCase to handle awaits.
    """
    async def asyncSetUp(self):
        # Initialize in simulated mode (no browser controller)
    ...
    async def test_valid_step(self):
        await self.env.reset()
        # Choose action 4
        obs, reward, terminated, truncated, info = await self.env.step(4)
    ...
    async def test_invalid_step(self):
        ...
        # First shot
        obs1, reward1, terminated1, truncated1, info1 = await self.env.step(2)
        if terminated1:
            # If goalie saved it, reset and hit index 2 once
            obs1, info1 = await self.env.reset()
            # Force target 2 to be hit to test invalid step
            self.env.grid_status[2] = 1.0
            
        # Second shot on the same index 2
        obs2, reward2, terminated2, truncated2, info2 = await self.env.step(2)
